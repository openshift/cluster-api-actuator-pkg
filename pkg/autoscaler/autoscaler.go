		...
		It("cordons node before draining during scale-down [Slow]", func() {
			...
			Eventually(func() (bool, error) {
				...
				for _, machine := range machines {
					...
					// Premature cordon: node is cordoned but has no deletion signal yet — this should not happen
					if node.Spec.Unschedulable {
						return false, fmt.Errorf("node %s is cordoned but has no deletion signal (no DeletionCandidate taint or delete annotation) — unexpected premature cordon", node.Name)
					}
				}
