...
var _ = Describe("Machine API GCP MachineSet", framework.LabelMAPI, framework.LabelDisruptive, Ordered, func() {
	...
	It("should have all Shielded VM options disabled when using nonUefi image", func() {
		...
		// Unmarshal the machine's provider spec to check Shielded VM configuration
		...
		// Verify that ShieldedInstanceConfig exists and all options are disabled
		Expect(machineProviderSpec.ShieldedInstanceConfig).ToNot(BeNil(), "ShieldedInstanceConfig should exist")
		Expect(machineProviderSpec.ShieldedInstanceConfig.SecureBoot).To(Equal(mapiv1.SecureBootPolicyDisabled), "SecureBoot should be disabled")
		Expect(machineProviderSpec.ShieldedInstanceConfig.VirtualizedTrustedPlatformModule).To(Equal(mapiv1.VirtualizedTrustedPlatformModulePolicyDisabled), "VirtualizedTrustedPlatformModule should be disabled")
		Expect(machineProviderSpec.ShieldedInstanceConfig.IntegrityMonitoring).To(Equal(mapiv1.IntegrityMonitoringPolicyDisabled), "IntegrityMonitoring should be disabled")
