...
var _ = Describe("[Feature:Machines] [AWS] MetadataServiceOptions", func() {
	...
	toCleanup := make([]*machinev1.MachineSet, 0, 3)
	...
	AfterEach(func() {
		Expect(framework.DeleteMachineSets(client, toCleanup...)).To(Succeed())
	})
	...
	assertIMDSavailability := func(machineset *machinev1.MachineSet, responseSubstring string) {
		By("Get node from machineset and spin a curl pod", func() {
			...
			logs, err := lastLog("curl-metadata", 10, false)
			Expect(err).ToNot(HaveOccurred())
	...
	It("should allow unauthorized requests to metadata service if metadataServiceOptions.authentication is empty", func() {
