...
func createAWSClient(oc *gatherer.CLI) {

}

// skipForCAPINotExist skip the test if capi doesn't exist
func SkipForCAPINotExist(oc *gatherer.CLI) {
	capi, err := oc.WithoutNamespace().Run("get").Args("deploy", "-n", CAPINamespace, "-o=jsonpath={.items[*].metadata.name}").Output()
	Expect(err).NotTo(HaveOccurred())
	if err != nil || len(capi) == 0 {
		Skip("Skip for cluster api is not deployed!")
	}
}
